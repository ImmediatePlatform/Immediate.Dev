...
export const GET: RequestHandler = async ({ url }) => {
	...
	entries.sort(
		(a, b) =>
			(sectionOrder.get(a.section) ?? Number.MAX_SAFE_INTEGER) -
				(sectionOrder.get(b.section) ?? Number.MAX_SAFE_INTEGER) || a.order - b.order
	);

	const siteTitle = docsConfig.site.title;
	const siteDesc = docsConfig.site.description;
	const baseUrl = docsConfig.site.url ?? url.origin;
	...
	for (const [section, sectionEntries] of sections) {
		const label =
			docsConfig.sidebar.find((s) => s.autogenerate?.directory === section)?.label ??
